...
async function login(page, email, password) {
  ...
  // Wait for navigation — first compile can take 15-20s in dev mode
...
async function logout(page) {
  ...
  await page.goto(`${BASE_URL}/accedi`, { waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(1000);
...
async function captureVolunteerScreenshots(page) {
  ...
  try {
    ...
    for (const href of uniqueHrefs) {
      try {
        // Visit each event to check capacity (use domcontentloaded for speed, with timeout)
        ...
        console.warn(`  Warning: failed to check event ${href}: ${err.message.split('\n')[0]}`);
      }
    }

    // 3. dettaglio-evento-disponibile
    if (availableEventUrl) {
      await page.goto(`${BASE_URL}${availableEventUrl}`, { waitUntil: 'domcontentloaded' });
      await page.waitForTimeout(1000);
    ...
    if (fullEventUrl) {
      await page.goto(`${BASE_URL}${fullEventUrl}`, { waitUntil: 'domcontentloaded' });
      await page.waitForTimeout(1000);
      await takeScreenshots(page, 'dettaglio-evento-completo', { fullPage: true });
    } else {
      // Fallback: try known full event (Evento speciale beneficenza, capacity 3, 3 confirmed + 2 waitlisted)
      console.log('  Trying known full event fallback...');
      await page.goto(`${BASE_URL}/calendario/f4d0f5e3-9d92-4dd5-b707-aa3a476d2ede`, { waitUntil: 'domcontentloaded' });
      await page.waitForTimeout(1500);
      const fallbackText = await page.textContent('body');
      if (fallbackText.includes('Evento al completo') || fallbackText.includes('lista d\'attesa') || fallbackText.includes('Iscriviti')) {
        fullEventUrl = '/calendario/f4d0f5e3-9d92-4dd5-b707-aa3a476d2ede';
        await takeScreenshots(page, 'dettaglio-evento-completo', { fullPage: true });
      } else {
        logMissing('dettaglio-evento-completo', 'No full event with waitlist found (fallback also failed)', 'Create a published event at full capacity with waitlist enabled');
      }
    }
    ...
    // 6. dialog-overlap — try overlapping event (Riunione settore Clown Terapia overlaps with Visita ospedaliera bambini on March 1)
    if (!capturedScreenshots.includes('dialog-overlap')) {
      try {
        // Navigate to the Riunione event which overlaps with Visita ospedaliera (volunteer is already registered for Visita)
        await page.goto(`${BASE_URL}/calendario/fb20368f-ac3e-4168-abd2-95462cc44f0f`, { waitUntil: 'domcontentloaded' });
        await page.waitForTimeout(1500);

        const overlapBtn = page.locator('button').filter({ hasText: /^Iscriviti$/ }).first();
        if (await overlapBtn.count() > 0) {
          await overlapBtn.click();
          await page.waitForTimeout(2000);

          const dialog = page.getByRole('dialog');
          if (await dialog.count() > 0) {
            // Click "Conferma iscrizione" to trigger overlap check
            const confermaBtn = dialog.getByRole('button', { name: /Conferma iscrizione/i });
            if (await confermaBtn.count() > 0) {
              await confermaBtn.click();
              await page.waitForTimeout(3000);
              ...
              const dialogText = await dialog.textContent();
              if (dialogText.includes('Sovrapposizione') || dialogText.includes('sovrapposizione') || dialogText.includes('sovrappone')) {
                await takeScreenshots(page, 'dialog-overlap');
              }
            }
            await closeDialog(page);
        ...
        if (!capturedScreenshots.includes('dialog-overlap')) {
          logMissing('dialog-overlap', 'Overlap warning did not appear', 'Events may not overlap or volunteer not registered for the other one');
        }
      } catch (err) {
        logMissing('dialog-overlap', err.message, 'Create overlapping events');
      }
    }
    ...
    for (const card of registeredCards) {
      await page.goto(`${BASE_URL}${card.href}`, { waitUntil: 'domcontentloaded' });
      await page.waitForTimeout(1000);
      ...
      if (hasAnnullaBtn) {
        // Check if this is a soon event (the cancel dialog will check for late warning)
        // Use all registered events — assign first to far, second to soon
        if (!registeredEventFar) {
          registeredEventFar = card.href;
        } else if (!registeredEventSoon) {
          registeredEventSoon = card.href;
        }
      }

      if (registeredEventFar && registeredEventSoon) break;
    }

    // 10. dialog-cancel-normal
    if (registeredEventFar) {
      await page.goto(`${BASE_URL}${registeredEventFar}`, { waitUntil: 'domcontentloaded' });
      await page.waitForTimeout(1000);
    ...
    // 11. dialog-cancel-late — try ALL registered events to find one that shows late warning
    if (!capturedScreenshots.includes('dialog-cancel-late')) {
      let foundLate = false;
      for (const card of registeredCards) {
        if (card.href === registeredEventFar && capturedScreenshots.includes('dialog-cancel-normal')) {
          // Skip the one we already used for normal cancel (it may no longer have the dialog)
          continue;
        }
        await page.goto(`${BASE_URL}${card.href}`, { waitUntil: 'domcontentloaded' });
        await page.waitForTimeout(1000);
        ...
        if (await cancelBtn.count() > 0) {
          ...
          if (await dialog.count() > 0) {
            const dialogText = await dialog.textContent();
            if (dialogText.includes('tardiva') || dialogText.includes('cancellazione tardiva')) {
      ...
      if (!foundLate) {
        // Fallback: try the known soon event directly (Animazione festa di quartiere, starts today, 24h deadline)
        console.log('  Trying late cancel fallback on known soon event...');
        await page.goto(`${BASE_URL}/calendario/275ed3ea-e2f3-4173-96c7-d52c51d29c6e`, { waitUntil: 'domcontentloaded' });
        await page.waitForTimeout(1500);
        ...
        if (await cancelBtn.count() > 0) {
          ...
          if (await dialog.count() > 0) {
            const dialogText = await dialog.textContent();
            console.log(`  Late cancel dialog text includes 'tardiva': ${dialogText.includes('tardiva')}`);
            if (dialogText.includes('tardiva') || dialogText.includes('cancellazione tardiva')) {
              await takeScreenshots(page, 'dialog-cancel-late');
              foundLate = true;
            }
        ...
        if (!foundLate) {
          logMissing('dialog-cancel-late', 'No cancel dialog showed late warning (fallback also tried)', 'Event may not have cancellation_deadline_hours set, or not within deadline');
        }
      }
    }
  ...
  console.log('\n── Profile Screenshots ──');
  try {
    await page.goto(`${BASE_URL}/profilo`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(2000);
...
async function captureAdminScreenshots(page) {
  ...
  try {
    await page.goto(`${BASE_URL}/admin/eventi`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(2000);
    ...
    if (await firstActionBtn.count() > 0) {
      ...
      if (await cloneItem.count() > 0) {
        ...
        if (await dialog.count() > 0) {
          ...
          // Try to click a future date in the calendar (react-day-picker uses data-day attr)
          const calendarDays = dialog.locator('button[data-day]:not([disabled])');
          const dayButtons = await calendarDays.all();
          console.log(`  Clone dialog day buttons: ${dayButtons.length}`);
          if (dayButtons.length > 0) {
            // Click a day that is not disabled
            await dayButtons[Math.min(5, dayButtons.length - 1)].click();
            await page.waitForTimeout(500);
          }
          ...
          const multiTab = dialog.getByRole('tab', { name: /Date multiple/i });
          if (await multiTab.count() > 0) {
            await multiTab.click();
            await page.waitForTimeout(1000);

            // Select 3 dates — use first 3 available, or navigate to next month if needed
            let calDays = await dialog.locator('button[data-day]:not([disabled])').all();
            console.log(`  Multi-date day buttons: ${calDays.length}`);

            // If fewer than 3, try navigating to next month
            if (calDays.length < 3) {
              const nextMonthBtn = dialog.locator('button[name="next-month"], button:has(svg)').last();
              if (await nextMonthBtn.count() > 0) {
                await nextMonthBtn.click();
                await page.waitForTimeout(500);
                calDays = await dialog.locator('button[data-day]:not([disabled])').all();
                console.log(`  After next month: ${calDays.length} day buttons`);
              }
            }

            let selected = 0;
            for (let i = 0; i < calDays.length && selected < 3; i++) {
              await calDays[i].click();
            ...
            if (selected >= 3) {
              ...
              logMissing('admin-clona-multipla', `Could not select 3 dates (only ${selected} clicked from ${calDays.length} available)`, 'Check calendar date picker');
            }
          } else {
  ...
  if (!capturedScreenshots.includes('admin-riepilogo-presenze-attivo') ||
    ...
    !capturedScreenshots.includes('admin-correzione-attiva') ||
    !capturedScreenshots.includes('admin-correzione-scaduta')) {
    try {
      // Collect event detail URLs from the admin events page first
      await page.goto(`${BASE_URL}/admin/eventi`, { waitUntil: 'domcontentloaded' });
      await page.waitForTimeout(1500);

      const attRows = await page.locator('table tbody tr').all();
      const attEventUrls = [];
      for (let i = 0; i < attRows.length; i++) {
        const actionBtn = attRows[i].locator('button').last();
        if (await actionBtn.count() > 0) {
          await actionBtn.click();
          await page.waitForTimeout(400);
          const viewLink = page.locator('a[href*="/admin/eventi/"]').first();
          if (await viewLink.count() > 0) {
            const href = await viewLink.getAttribute('href');
            if (href) attEventUrls.push(href);
          }
          await page.keyboard.press('Escape');
          await page.waitForTimeout(200);
        }
      }
      console.log(`  Found ${attEventUrls.length} admin event URLs to check for attendance`);

      // Visit each event detail page
      for (const eventUrl of attEventUrls) {
        await page.goto(`${BASE_URL}${eventUrl}`, { waitUntil: 'domcontentloaded' });
        await page.waitForTimeout(1500);

        const bodyText = await page.textContent('body');
        const hasAttendance = bodyText.includes('Presenti') || bodyText.includes('Assenti') || bodyText.includes('Riepilogo presenze');

        if (hasAttendance) {
          console.log(`  Found attendance data at ${eventUrl}`);
          const hasGraceActive = bodyText.includes('Puoi correggere') || bodyText.includes('correggere le presenze entro');
          const hasGraceExpired = bodyText.includes('scaduto') || bodyText.includes('è scaduto');

          if (hasGraceActive && !capturedScreenshots.includes('admin-riepilogo-presenze-attivo')) {
            await takeScreenshots(page, 'admin-riepilogo-presenze-attivo', { fullPage: true });

            // Also capture correction dropdown
            if (!capturedScreenshots.includes('admin-correzione-attiva')) {
              const correggiTriggers = page.getByRole('combobox');
              const triggerCount = await correggiTriggers.count();
              console.log(`  Combobox triggers for correction: ${triggerCount}`);
              if (triggerCount > 0) {
          ...
          if (hasGraceExpired && !capturedScreenshots.includes('admin-riepilogo-presenze-scaduto')) {
            await takeScreenshots(page, 'admin-riepilogo-presenze-scaduto', { fullPage: true });
          }

          if (hasGraceExpired && !capturedScreenshots.includes('admin-correzione-scaduta')) {
  ...
  try {
    await page.goto(`${BASE_URL}/admin/utenti`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(2000);
  ...
  try {
    await page.goto(`${BASE_URL}/admin/export`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(2000);
...
async function main() {
  ...
  try {
    // Warmup: pre-compile key pages to avoid slow first-load timeouts
    console.log('\n── Warming up dev server (pre-compiling pages) ──');
    const warmupContext = await browser.newContext({ viewport: DESKTOP });
    const warmupPage = await warmupContext.newPage();
    const warmupUrls = ['/accedi', '/calendario', '/dashboard', '/profilo',
      '/admin/eventi', '/admin/utenti', '/admin/audit', '/admin/export'];
    for (const path of warmupUrls) {
      try {
        await warmupPage.goto(`${BASE_URL}${path}`, { waitUntil: 'domcontentloaded', timeout: 60000 });
        await warmupPage.waitForTimeout(500);
        console.log(`  Warmed: ${path}`);
      } catch (err) {
        console.warn(`  Warmup failed for ${path}: ${err.message.split('\n')[0]}`);
      }
    }
